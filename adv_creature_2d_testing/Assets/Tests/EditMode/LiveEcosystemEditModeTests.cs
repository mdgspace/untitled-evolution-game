...
{
    [Test]
    public void V20WidthsDepthsAndHeadPartitionsAreExact()
    {
        ...
        Assert.AreEqual(4,NativeCreatureModel.M2MlpHiddenLayers);Assert.AreEqual(5,NativeCreatureModel.M3MlpHiddenLayers);
        NativeBrainWeights policy=NativeBrainWeights.Create(1);NativeDynamicsWeights dynamics=NativeDynamicsWeights.Create(2);
        Assert.AreEqual(28*32,policy.m1Input.Length);Assert.AreEqual(10*32,policy.m2Context.Length);Assert.AreEqual(7*32,policy.m2Joint.Length);Assert.AreEqual(16*32,policy.m2Position.Length);Assert.AreEqual(32*32,policy.m2Deep2.Length);Assert.AreEqual(5*32,policy.m2StepEmbedding.Length);Assert.AreEqual(32,policy.m2Output.Length);
        Assert.AreEqual(NativeCreatureModel.M2ParameterCount,policy.M2Parameters().Sum(p=>p.Value.Length));
        Assert.AreEqual(12*32,dynamics.joint.Length);Assert.AreEqual(5*32,dynamics.horizonQueries.Length);Assert.AreEqual(32*32,dynamics.deep3.Length);Assert.AreEqual(32*9,dynamics.output.Length);
    }
